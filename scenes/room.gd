...
func _ready()->void:
	for i in range(0, width):
		for j in range(0, height):
			...
			if i == 0 and running_walls % 8 == running_walls: # left wall
				tileMap.set_cell(Vector2(start_point.x * width + i, start_point.y * height + j), ss_i, wall_sample)
			running_walls = running_walls % 8
			if  j == height - 1 and running_walls % 4 == running_walls: # upper wall
				tileMap.set_cell(Vector2(start_point.x * width + i, start_point.y * height + j), ss_i, wall_sample)
			running_walls = running_walls % 4
			if i == width - 1 and running_walls % 2 == running_walls: # right wall
				tileMap.set_cell(Vector2(start_point.x * width + i, start_point.y * height + j), ss_i, wall_sample)
			running_walls = running_walls % 2
			if j == 0 and running_walls % 1 == running_walls: # bottom wall
				tileMap.set_cell(Vector2(start_point.x * width + i, start_point.y * height + j), ss_i, wall_sample)
			else:
				pass
	if door_dir == "up":
		...
		tileMap.set_cell(Vector2(start_point.x * width + 5 , start_point.y * height + height), ss_i, floor_sample)#tileMap.set_cell(Vector2(i,j),1,Vector2(3,1)) # floor
		tileMap.set_cell(Vector2(start_point.x * width + 4 , start_point.y * height + height), ss_i, floor_sample)#tileMap.set_cell(Vector2(i,j),1,Vector2(3,1)) # floor
	elif door_dir == "left":
		tileMap.set_cell(Vector2(start_point.x * width + 0 , start_point.y * height + 5), ss_i, floor_sample)#tileMap.set_cell(Vector2(i,j),1,Vector2(3,1)) # floor
		tileMap.set_cell(Vector2(start_point.x * width + 0 , start_point.y * height + 4), ss_i, floor_sample)#tileMap.set_cell(Vector2(i,j),1,Vector2(3,1)) # floorpass #	if i == start.x or j == start.y or i == end.x -1 or j == end.y -1 :
	elif door_dir == "right":
